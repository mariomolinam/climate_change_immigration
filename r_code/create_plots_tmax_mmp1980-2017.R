...
tmax.mmp.monthly = tmax.mmp[,"geocode"]

for(y in 1:length(nmonths) ){
  month = nmonths[[y]]
  for(m in 2:length(month)){
    range = (month[m-1]):(month[m]-1); range
    sum.monthly = as.matrix( apply(tmax.mmp[,2:ncol][,range], 1, mean) )
    tmax.mmp.monthly = cbind( tmax.mmp.monthly, sum.monthly )   
  }
}

# calculate the monthly average (monthly norm) of prcp across communities
ncol = dim(tmax.mmp.monthly)[2]
norm.community = rowMeans( tmax.mmp.monthly[,2:ncol] )
sd.prcp.community = apply(tmax.mmp.monthly[,2:ncol], 1, sd)
dev.community = tmax.mmp.monthly[,2:ncol] - norm.community
community.dev.mean = cbind(tmax.mmp.monthly[,1], dev.community)
# add column names
...
year.month = paste0("tmax-",year.month)

colnames(community.dev.mean) = c("geocode", year.month)

# save community-level norm deviation between 1980 and 2017
setwd(path.mmp)
write.csv(community.dev.mean, "tmax_monthly_dev-norm_1980-2017.csv", row.names = FALSE)

# create plot for rain
setwd(path.git)
jpeg('./results/tmax_mmp_1980-2017.jpeg', height = 10000, width = 12500, res=400, pointsize=22)
