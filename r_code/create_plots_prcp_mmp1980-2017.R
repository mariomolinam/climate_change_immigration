...
prcp.mmp.monthly = prcp.mmp[,"geocode"]

for(y in 1:length(nmonths) ){
  month = nmonths[[y]]
  for(m in 2:length(month)){
    range = (month[m-1]):(month[m]-1); range
    sum.monthly = as.matrix( apply(prcp.mmp[,2:ncol][,range], 1, sum) )
    prcp.mmp.monthly = cbind( prcp.mmp.monthly, sum.monthly )   
  }
}

# calculate the monthly average (monthly norm) of prcp across communities
ncol = dim(prcp.mmp.monthly)[2]
norm.community = rowMeans( prcp.mmp.monthly[,2:ncol] )
sd.prcp.community = apply(prcp.mmp.monthly[,2:ncol], 1, sd)
dev.community = prcp.mmp.monthly[,2:ncol] - norm.community
community.dev.mean = cbind(prcp.mmp.monthly[,1], dev.community)
# add column names
...
setwd(path.mmp)
write.csv(community.dev.mean, "prcp_monthly_dev-norm_1980-2017.csv")

# create plot for rain
setwd(path.git)
jpeg('./results/prcp_mmp_1980-2017.jpeg', height = 10000, width = 12500, res=400, pointsize=22)
...
for(comm in 1:nrow){
  ...
  # create plot frame
  plot(time, y, type='n',  ylim = c(-200,1000), las=1, ylab = "", xlab="", axes=FALSE)
  ...
  axis(2, at=seq(-200,1000, 100), cex.axis=0.45, las=1)
  ...
  # legend
  legend(350, 990, legend=c("1 sd", "2 sd"), col="red", lty=c(2,3), bty="n", cex = 0.3, lwd=c(0.5,0.3))
}
dev.off()
