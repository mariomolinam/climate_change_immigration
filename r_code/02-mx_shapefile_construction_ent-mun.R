...
for(geo in geography){
  ...
  if(! paste0("mx_sh_",geo) %in% list.files()) dir.create(paste0("mx_sh_",geo)) # create directory to store MX shapefiles
  ...
  for(i in 1:length(all.files)){
    ...
    pattern = paste0( substr(file.name, 1,2), geo) # , 'territorio_insular') ) # 'a' excluded
    # remove island query if non-existent
    # if( length(grep('territorio_insular', files)) == 0 ) pattern = pattern[-2]
    ...
    writeOGR(obj=shapefile, dsn=paste0('mx_sh_',geo,'/'), layer=paste0('mx', substr(file.name, 1,2)), driver = 'ESRI Shapefile')  
    rm(shapefile, f.read)
    gc()
    cat('Done!', '\n')
  ...
  mx.layers = list.files(paste0('mx_sh_',geo,'/'))
  mx.layers = unique(sub('[[:punct:]].*', '', mx.layers))
  mx.shapefile = do.call(raster::bind, lapply(mx.layers, function(x) readOGR(paste0('mx_sh_',geo,'/.'), layer = x)))
  summary(mx.shapefile)
  
  # plot(mx.shapefile)
  
  # save shapefile
  writeOGR(obj=mx.shapefile, dsn='.', layer=paste0("mx_",geo), driver = 'ESRI Shapefile')  
}
